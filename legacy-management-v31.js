(function(){
  ...
  function totals(){
    const out={};
    function row(name){name=norm(name);return out[name]||(out[name]={name,totalG:0,fridgeG:0,freezerG:0,roomG:0,cubeG:0,count:0})}
    for(const [k,v] of Object.entries(inventory||{})){
      const r=row(invName(k)),g=grams(v),q=Math.max(0,Number(v.qty)||0),loc=v.location||'냉동';
      r.totalG+=g;if(loc==='냉장')r.fridgeG+=g;else if(loc==='실온')r.roomG+=g;else r.freezerG+=g;
      if(!g&&v.unit==='개')r.count+=q;
    }
    for(const b of cubes()){
      const r=row(b.ingredient),g=Math.max(0,(Number(b.unitG)||0)*(Number(b.remainingCount)||0));r.totalG+=g;r.freezerG+=g;r.cubeG+=g;
    }
    return out;
  }
  function totalSummary(){
    ...
    return rows.map(r=>`<div class="inventory-row"><div style="flex:1"><b>${esc(r.name)}</b><div class="hint">${r.fridgeG?`냉장 ${Math.ceil(r.fridgeG)}g · `:''}${r.freezerG?`냉동 ${Math.ceil(r.freezerG)}g · `:''}${r.roomG?`실온 ${Math.ceil(r.roomG)}g`:''}${r.count?`${r.totalG?' · ':''}${Math.ceil(r.count)}개`:''}</div></div><b>${r.totalG?`${Math.ceil(r.totalG)}g`: `${Math.ceil(r.count)}개`}</b></div>`).join('');
  }
  ...
  function groupRows(loc){let html=Object.entries(inventory||{}).filter(([,v])=>(v.location||'냉동')===loc).map(([k,v])=>rawRow(k,v)).join('');if(loc==='냉동')html+=cubes().map(cubeRow).join('');return html||`<p class="hint">${loc} 보관 재고가 없어요.</p>`}
  function stockView(){
    const opts=inventoryKeys().map(k=>`<option value="${esc(k)}">${esc(invName(k))}</option>`).join('');
    return `${nav()}<div class="sec"><h2>1단계 · 재고관리</h2><span class="more">총량이 핵심</span></div>
      <p class="hint">가장 먼저 <b>재료별 총 보유량</b>을 보여주고, 아래에서 냉장·냉동·실온 위치만 구분해 확인해요.</p>
      ...
      <div class="sec"><h2>소분 냉동 추가</h2></div><div class="card"><p class="hint">20g × 6개처럼 소분해 둔 것만 입력해요. 총 보유량에는 자동 합산돼요.</p><div class="fld"><label>재료</label><select id="mg30CubeKey">${opts}</select></div><div style="display:grid;grid-template-columns:1fr 1fr;gap:8px"><div class="fld"><label>1개 기준량(g)</label><input id="mg30CubeUnit" type="number" min="1" step="1" value="${unitFor('rice')}"></div><div class="fld"><label>현재 개수</label><input id="mg30CubeCount" type="number" min="0" step="1" placeholder="예: 6"></div></div><div class="fld"><label>제조일</label><input id="mg30CubeDate" type="date" value="${today}"></div><div class="btnrow"><button class="btn pri" data-mg30="cubeadd">냉동에 추가</button></div></div>`;
  }

  function requirements(w){
    const t=weekList(w.start,w.count);let rice=0;
    for(let d=0;d<w.count;d++){const on=addD(w.start,d);for(let i=0;i<3;i++)if(mObj(on,i))rice+=stage(on).rice}
    if(rice)t['밥 (조리 후)']={c:'e',g:rice,n:0};return t;
  }
  function allocation(){
