(function(){
  ...
  function requirements(start,count){const t=weekList(start,count);let rice=0;for(let d=0;d<count;d++){const on=addD(start,d),pv=window.__ppeuniSafeV57&&window.__ppeuniSafeV57.entry(on);for(let i=0;i<3;i++)if(!pv&&mObj(on,i))rice+=stage(on).rice}if(rice)t['밥 (조리 후)']={c:'e',g:rice,n:0};return t}
