(function(){
  ...
  function allocation(){
    ...
    for(const w of windows()){
      ...
      for(const [name0,v] of Object.entries(requirements(w.start,w.count))){
        ...
        if(needMaxG>0){const before=byLoc(p.grams,'g'),available=Object.values(before).reduce((a,b)=>a+(Number(b)||0),0),c=consumeVirtual(p.grams,needMaxG,'g'),countStock=(p.counts||[]).reduce((a,x)=>a+(Number(x.n)||0),0),unitMismatch=c.left>0&&countStock>0;out[name]={name,needG,needMaxG,beforeG:before,buyMinG:Math.max(0,needG-available),buyG:unitMismatch?0:Math.max(0,c.left),range:needMaxG>needG,unknown:unitMismatch,unitMismatch,countStock}}
        ...
        else out[name]={name,needN,unknown:true,recipe:!!v.recipe};
      }
      result[w.id]=out;
    }
    return result;
  ...
  function shoppingRows(w,rows){if(isDone(w))return `<p class="hint"><b>${w.title} 제작 완료.</b> 사용 재고는 이미 차감됐어요.</p>`;const list=Object.values(rows).filter(x=>x.unknown||(x.buyG||0)>0||(x.buyN||0)>0);if(!list.length)return '<p class="hint"><b>추가 구매 없음.</b> 현재 남은 재고로 준비 가능해요.</p>';return list.map(x=>{const on=checked(w,x.name),isCount=x.buyN!==undefined,qty=x.unknown?(x.recipe?'레시피 확인':x.unitMismatch?'g 환산 필요':'수량 확인'):x.range?`${Math.ceil(x.buyMinG||0)}~${Math.ceil(x.buyG||0)}g`:`${Math.ceil(isCount?x.buyN:x.buyG)}${isCount?'개':'g'}`,need=x.unknown?(x.recipe?`${x.needN||''}회 · 원본 레시피`:x.unitMismatch?`${Math.ceil(x.needG)}${x.needMaxG>x.needG?'~'+Math.ceil(x.needMaxG):''}g`:`${x.needN||''}회`):x.range?`${Math.ceil(x.needG)}~${Math.ceil(x.needMaxG)}g`:`${Math.ceil(isCount?x.needN:x.needG)}${isCount?'개':'g'}`,stock=x.unknown?(x.recipe?'원재료 분량 확인 필요':x.unitMismatch?`개수 재고 ${Math.ceil(x.countStock||0)}개 · 1개당 g 입력 필요`:'재고 단위 확인 필요'):locText(isCount?x.beforeN:x.beforeG,isCount?'개':'g');return `<div class="shop${on?' on':''}" data-v33check="${w.id}|${encodeURIComponent(x.name)}" style="align-items:flex-start"><div class="bx" style="margin-top:5px"></div><div class="nm"><b>${esc(x.name)}</b><div class="hint">필요 ${esc(need)}</div><div class="hint"><b>현재 남은 재고</b> · ${esc(stock)}</div></div><div style="min-width:96px;text-align:center;background:var(--peach-s);border-radius:14px;padding:8px 7px;color:var(--peach)"><div style="font-size:10.5px;font-weight:800">${on?'구매완료':'추가 구매'}</div><div style="font-size:20px;font-weight:900;line-height:1.15;margin-top:2px">${esc(qty)}</div></div></div>`}).join('')}
  ...
  function measurableShortages(start,count){const snap=stockSnapshot(),miss=[];for(const [name0,v] of Object.entries(requirements(start,count))){const name=norm(name0),needG=Math.max(0,Number(v.gMax)||Number(v.g)||0),needN=Math.max(0,Number(v.n)||0),r=snap[name]||{g:0,n:0};if(needG>0&&r.g+1e-9<needG)miss.push(r.n>0?`${name} 개수 재고 ${Math.ceil(r.n)}개 · 1개당 g 입력 필요`:`${name} ${Math.ceil(needG-r.g)}g`);else if(name==='계란'&&needN>0&&r.n+1e-9<needN)miss.push(`${name} ${Math.ceil(needN-r.n)}개`)}return miss}
  function deductBatch(start,count){
    if(markers[start])return {ok:true,already:true};
    const req=requirements(start,count),recipes=Object.entries(req).filter(([,v])=>v?.recipe),ranges=Object.entries(req).filter(([,v])=>(Number(v.gMax)||0)>(Number(v.g)||0));
    if(recipes.length)return{ok:false,recipe:true,missing:recipes.slice(0,3).map(([n])=>`${norm(n)} · 원본 레시피 원재료 확인`)};
