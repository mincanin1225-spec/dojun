(function(){
  ...
  function allocation(){
    ...
    for(const w of windows()){
      ...
      for(const [name0,v] of Object.entries(requirements(w.start,w.count))){
        ...
        if(needG>0){const before=byLoc(p.grams,'g'),c=consumeVirtual(p.grams,needG,'g');out[name]={name,needG,beforeG:before,buyG:Math.max(0,c.left),unknown:false}}
  ...
  function shoppingRows(w,rows){if(isDone(w))return `<p class="hint"><b>${w.title} 제작 완료.</b> 사용 재고는 이미 차감됐어요.</p>`;const list=Object.values(rows).filter(x=>x.unknown||(x.buyG||0)>0||(x.buyN||0)>0);if(!list.length)return '<p class="hint"><b>추가 구매 없음.</b> 현재 남은 재고로 준비 가능해요.</p>';return list.map(x=>{const on=checked(w,x.name),isCount=x.buyN!==undefined,qty=x.unknown?'수량 확인':`${Math.ceil(isCount?x.buyN:x.buyG)}${isCount?'개':'g'}`,need=x.unknown?`${x.needN||''}회`:`${Math.ceil(isCount?x.needN:x.needG)}${isCount?'개':'g'}`,stock=x.unknown?'재고 단위 확인 필요':locText(isCount?x.beforeN:x.beforeG,isCount?'개':'g');return `<div class="shop${on?' on':''}" data-v33check="${w.id}|${encodeURIComponent(x.name)}" style="align-items:flex-start"><div class="bx" style="margin-top:5px"></div><div class="nm"><b>${esc(x.name)}</b><div class="hint">필요 ${esc(need)}</div><div class="hint"><b>현재 남은 재고</b> · ${esc(stock)}</div></div><div style="min-width:96px;text-align:center;background:var(--peach-s);border-radius:14px;padding:8px 7px;color:var(--peach)"><div style="font-size:10.5px;font-weight:800">${on?'구매완료':'추가 구매'}</div><div style="font-size:20px;font-weight:900;line-height:1.15;margin-top:2px">${esc(qty)}</div></div></div>`}).join('')}
