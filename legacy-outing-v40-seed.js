(function(){
  ...
  const DISPLAY_VER=window.__DOJUN_RELEASE?.version||'v69';
  const NOW=Date.now();
  const SEED=[
  ...
  function seedPlaces(){
    if(localStorage.getItem(STORAGE_KEY)!==null){window.__outingPlaces=readRaw();return 0}
    const existing=[],keys=new Set();
    let added=0;
