(function(root){
  ...
  function pool(state,raw=true){
    ...
    for(const kind of ['prepared','cubes']) (state[kind]||[]).forEach((lot,i)=>{
      if(amount(lot))rows.push({kind,i,id:id(lot,i),name:norm(lot.name||lot.ingredient),g:amount(lot),date:lot.madeDate||'',unitG:Number(lot.unitG)});
    });
    if(raw)for(const [k,v]of Object.entries(state.raw||{})){
      const g=v.unit==='g'?Number(v.qty):Number(v.qty)*Number(v.gramsPerUnit);
      if(positive(g))rows.push({kind:'raw',i:k,id:k,name:norm(v.displayName||k),g,date:v.madeDate||'',unitG:1});
    }
