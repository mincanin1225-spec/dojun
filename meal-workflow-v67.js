 ...
 function prepTasks(rows,start){
   ...
   for(const r of rows){
     for(const n of r.needs||[]){
       ...
       const x=map[name]||(map[name]={name,totalG:0,unknown:false,mealKeys:new Set()});
       x.mealKeys.add(r.meal.key);
       if(n.buyG===null)x.unknown=true;else if(Number(n.buyG)>0)x.totalG+=Number(n.buyG);
     }
   }
   return Object.values(map).filter(x=>x.unknown||x.totalG>1e-6).map(x=>{
     const unitG=prepUnit(x.name),count=unitG&&x.totalG>0?Math.ceil(x.totalG/unitG):null,totalG=Math.round(x.totalG*10)/10,meals=x.mealKeys.size;
     const key=[start,x.name,totalG,unitG||0,count||0,meals].join('|');
     return{name:x.name,totalG,unitG,count,meals,unknown:x.unknown,key};
   });
 }
 function prepTaskRow(t,done){
   const qty=t.unitG&&t.count?('<b>'+t.unitG+'g × '+t.count+'개</b>'):(t.totalG>0?'<b>총 '+t.totalG+'g</b>':'<b>분량 확인 필요</b>');
   const meta=(t.meals?t.meals+'끼 · ':'')+(t.totalG>0?'필요 '+t.totalG+'g':'원본 분량 확인 필요');
   return '<div class="shop'+(done?' on':'')+'" data-v67-prepdone="'+encodeURIComponent(t.key)+'" style="cursor:pointer"><div class="bx"></div><div class="nm"><b>'+esc(t.name)+'</b><div>'+qty+'</div><div class="hint">'+esc(meta)+'</div></div><div class="qt">'+(done?'만들기 완료':'미완료')+'</div></div>';
 }
