(function(root,factory){
  ...
  function assertVerifiedMeal(meal){
    ...
    if(!meal.sourceRef)throw new Error('sourceRef required');
  ...
  function mealsInRange(plan,start,count){
    const out=[];
    for(let i=0;i<count;i++){
      const date=addDays(start,i),day=plan?.[date]||{};
      for(const slot of ['breakfast','lunch','dinner'])if(day[slot])out.push({date,slot,meal:assertVerifiedMeal(day[slot])});
    }
    return out;
  }
  function buildPrepList(plan,start,count,batches=[]){
    const meals=mealsInRange(plan,start,count),needs={};
  ...
  function completeMealOnce(state,date,slot,meal){
    meal=assertVerifiedMeal(meal); const key=planMealKey(date,slot),done={...(state.done||{})};
    ...
    done[key]={completedAt:new Date().toISOString(),sourceRef:meal.sourceRef,shortages};
    return {...state,batches,done,alreadyDone:false,shortages};
  }
  return {iso,parse,addDays,ageMonths,ppeuniStageForAge,mondayOf,prepWindowsForWeek,cleanBatch,batchAvailableG,stockFor,consumeFIFO,remakeSuggestion,planMealKey,assertVerifiedMeal,mealsInRange,buildPrepList,completeMealOnce};
});