const fs=require('fs');
const assert=require('assert');
const index=fs.readFileSync('index.html','utf8');
const sw=fs.readFileSync('sw.js','utf8');
assert(index.includes('<iframe id="app"'),'stable shell must use legacy app iframe');
assert(index.includes('legacy-ui-v47.js?v=20260916-v47&r=20260919-v58'),'v47 UI patch must stay in the original order with the v58 cache-bust');
assert(index.includes('visibility:visible'),'base app must be visible immediately');
assert(index.includes('v58-ppeuni-data-script')&&index.includes('v58-ppeuni-safe-script'),'verified Ppeuni integration must load after the stable UI shell');
assert(sw.includes("const CACHE='dojun-pwa-v58'"),'v58 service worker marker missing');
console.log('PASS: stable enhanced shell remains preserved under v58');
