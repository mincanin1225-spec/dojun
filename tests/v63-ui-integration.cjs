...
for(const f of ['meal-stock-v63.js','meal-workflow-v65.js'])vm.runInContext(fs.readFileSync(f,'utf8'),ctx);
...
vm.runInContext(fs.readFileSync('meal-workflow-v65.js','utf8'),ctx);assert.equal(ctx.__MEAL_WORKFLOW_V63.model('2026-09-21',0).ingredients[1].g,1);
