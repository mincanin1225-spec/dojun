...
new Function(flow);
assert(index.includes("./meal-workflow-v70.js?r=20260924-v80-makelabel1"),'index must load the new physical workflow filename');
...
assert(index.includes("const RELEASE='20260924-v80-makelabel1'"),'index must carry the current PWA release marker');
const loaderReleases=[...index.matchAll(/[?&]r=([0-9A-Za-z-]+)/g)].map(m=>m[1]);
assert.deepEqual([...new Set(loaderReleases)],['20260924-v80-makelabel1'],'all loader cache-busters must match the current release');
assert(index.includes("./legacy-management-v78-bundle.js?r=20260924-v80-makelabel1"),'boot must load the consolidated management bundle');
...
assert(sw.includes("const CACHE='dojun-pwa-v80-makelabel1'"),'new cache generation missing');
