...
new Function(flow);
assert(index.includes("./meal-workflow-v70.js?r=20260922-v70-prepprogress1"),'index must load the new physical workflow filename');
...
assert(sw.includes("const CRITICAL=['/legacy-v70.html','/meal-stock-v66.js','/meal-workflow-v70.js','/legacy-management-v33.js','/health-schedule-v43.js','/legacy-health-v43.js','/legacy-outing-v36.js','/legacy-outing-v40-seed.js','/legacy-outing-v41-no-embedded-map.js']"),'critical active assets must use the network-first path');
assert(sw.includes("fetch(req,{cache:'no-store'})"),'critical assets must bypass stale HTTP/cache reads');
assert(index.includes("const RELEASE='20260922-v70-prepprogress1'"),'index must carry the current PWA release marker');
...
assert(sw.includes("const CACHE='dojun-pwa-v70-prepprogress1'"),'new cache generation missing');
