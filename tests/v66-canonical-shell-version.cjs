...
assert(index.includes('<iframe id="app" src="./legacy-v68.html?r=20260921-v68"'),'index must load the new physical v68 shell');
...
assert(sw.includes("'./legacy-v68.html'"),'service worker must cache the physical v68 shell');
assert(sw.includes("const CACHE='dojun-pwa-v68-recipeui1'"),'v68 shell cache marker missing');
