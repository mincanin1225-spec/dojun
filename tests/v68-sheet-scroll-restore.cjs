...
assert(index.includes('./legacy-v70.html?r=20260922-v72-recordonly4'),'index must load the scroll-restoring physical shell');
assert(sw.includes("'./legacy-v70.html'"),'service worker must precache the scroll-restoring shell');
assert(sw.includes("const CACHE='dojun-pwa-v72-recordonly4'"),'scroll-restoring cache generation missing');
