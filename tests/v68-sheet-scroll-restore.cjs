...
assert(index.includes('./legacy-v70.html?r=20260922-v71-recordonly2'),'index must load the scroll-restoring physical shell');
assert(sw.includes("'./legacy-v70.html'"),'service worker must precache the scroll-restoring shell');
assert(sw.includes("const CACHE='dojun-pwa-v71-recordonly2'"),'scroll-restoring cache generation missing');
