...
assert(shell.includes("function dishSVG(m,w,label='')"),'meal cards must have a text-based visual fallback when Ppeuni suppresses mObj');
...
assert(index.includes('./legacy-v70.html?r=20260923-v77-mealvisual1'),'index must load v70 shell');
assert(index.includes('./meal-workflow-v70.js?r=20260923-v76-calmark1'),'index must load v70 workflow');
assert(shell.includes("name:'도준이키우기',version:'v77'"),'canonical version must be v70');
assert(sw.includes("const CACHE='dojun-pwa-v77-mealvisual1'"),'v70 cache missing');
...
console.log('PASS: v77 prep cycle keeps stock -> shopping -> make -> remaining stock, with optional feeding records');
