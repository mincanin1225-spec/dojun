...
assert(stock.includes("const round=x=>Math.round(x*1000)/1000;"),'stock math must normalize sub-milligram floating noise');
...
assert(index.includes('legacy-management-v33.js?v=20260916-v40&r=20260922-v70-invsync1'),'inventory renderer must use a fresh release URL');
assert(index.includes("const RELEASE='20260922-v70-invsync1'"),'PWA release marker must refresh');
assert(sw.includes("const CACHE='dojun-pwa-v70-invsync1'"),'PWA cache marker must refresh');
