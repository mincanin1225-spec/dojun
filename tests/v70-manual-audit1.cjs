...
assert(shell.includes("['shop','준비','shop']"),'base bottom tab must say 준비 from first paint');
assert(shell.includes('준비 탭에서 재고 확인 → 장보기 → 만들기 순서로 이어서 준비해요.'),'calendar weekend copy must point to the preparation flow');
...
assert(index.includes('legacy-v70.html?r=20260924-v80-simpleprep1'),'active shell release must refresh');
assert(index.includes('legacy-health-v43.js?v=20260916-v43&r=20260924-v80-simpleprep1'),'health module release must refresh');
assert(index.includes("const RELEASE='20260924-v80-simpleprep1'"),'PWA release marker must refresh');
assert(sw.includes("const CACHE='dojun-pwa-v80-simpleprep1'"),'PWA cache marker must refresh');
